...
Dataset
FileDescriptiondata/sf_boba_tea_raw.csvOriginal uncleaned dataset (606 rows)data/sf_boba_tea_cleaned.csvFinal cleaned dataset (595 rows)
...
Reasoning: Replacing invalid ratings with an arbitrary value (e.g., 5) fabricates data without evidence. Since we cannot verify the true ratings without returning to the source, deletion is the more honest and defensible approach. See cleaning log for full reasoning.
Rows removed: 9
...
Results
MetricBeforeAfterTotal rows606595Duplicate rows30Invalid ratings (>5)90Lat-long formatCombined stringSeparate numeric columns
