...
**Raw dataset:** 606 rows × 6 columns  
...
**Final decision:** Delete rows. The dataset has 606 rows — losing 9 (1.5%) is an acceptable tradeoff for data integrity. The agency's goal is to find top-rated shops, so shops with unverifiable ratings should not be included.
...
**Rows removed:** 11 total across all steps  
**Final rows remaining:** 595  
